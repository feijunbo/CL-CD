...
# 微调后参数存放位置
SAVE_PATH = './saved_model/zerorc_sup_paddle.pt'
...
class TrainDataset(Dataset):
    ...
    def text_2_id(self, text):
        return tokenizer([text[0], text[1], text[2]], max_length=MAXLEN, 
                         truncation=True, padding='max_length', return_tensors='pd', return_attention_mask=True, return_token_type_ids=True)
...
class TestDataset(Dataset):
    ...
    def text_2_id(self, text: str):
        return tokenizer(text, max_length=MAXLEN, truncation=True, padding='max_length', return_tensors='pd', return_attention_mask=True, return_token_type_ids=True)
...
class SimcseModel(nn.Layer):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        outputs, pooled_out = self.bert(input_ids, output_hidden_states=True)

        if self.pooling == 'cls':
            return outputs[:, 0]  # [batch, 768]
        
        if self.pooling == 'pooler':
            return pooled_out            # [batch, 768]
        
        if self.pooling == 'last-avg':
            last = outputs[-1].transpose([0, 1, 2])    # [batch, 768, seqlen]
            return F.avg_pool1d(last, kernel_size=last.shape[-1]).squeeze(-1)       # [batch, 768]
        
        if self.pooling == 'first-last-avg':
            first = outputs[0].transpose([0, 1, 2])    # [batch, 768, seqlen]
            last = outputs[-1].transpose([0, 1, 2])    # [batch, 768, seqlen]                   
            first_avg = F.avg_pool1d(first, kernel_size=last.shape[-1]).squeeze(-1) # [batch, 768]
            last_avg = F.avg_pool1d(last, kernel_size=last.shape[-1]).squeeze(-1)   # [batch, 768]
            avg = paddle.concat((first_avg.unsqueeze(1), last_avg.unsqueeze(1)), axis=1)     # [batch, 2, 768]
            return F.avg_pool1d(avg.transpose([0, 1, 2]), kernel_size=2).squeeze(-1)     # [batch, 768]
...
def simcse_sup_loss(y_pred):
    ...
    y_true = paddle.arange(y_pred.shape[0])
    use_row = paddle.where((y_true + 1) % 3 != 0)[0]
    y_true = (use_row - use_row % 3 * 2) + 1
    # batch内两两计算相似度, 得到相似度矩阵(对角矩阵)
    sim = F.cosine_similarity(y_pred.unsqueeze(1), y_pred.unsqueeze(0), axis=-1)
   
    # 将相似度矩阵对角线置为很小的值, 消除自身的影响
    sim = sim - paddle.eye(y_pred.shape[0]) * 1e12
    # 选取有效的行
    sim = paddle.index_select(sim, use_row, 0)
    # 相似度矩阵除以温度系数
    sim = sim / 0.05
...
def eval(model, dataloader, label_sents):
    ...
    """
    model.eval()
    sim_array = paddle.to_tensor([])
    label_array = np.array([])
    target = tokenizer(label_sents, max_length=MAXLEN, truncation=True, padding='max_length', return_tensors='pd', return_attention_mask=True, return_token_type_ids=True)
    with paddle.no_grad():
        ...
        for source, label in tqdm(dataloader):
            ...
            # concat
            sim = F.cosine_similarity(source_pred, target_pred, axis=-1)
            sim_array = paddle.concat((sim_array, sim), axis=0)            
            label_array = np.append(label_array, np.array(label))

    pred_array = sim_array.argmax(-1).numpy()
...
def tsne(model, dataloader, label_sents):
    ...
    with paddle.no_grad():
        ...
        target_token_type_ids = target.get('token_type_ids').squeeze(1)
        target_pred = model(target_input_ids, target_attention_mask, target_token_type_ids)
        embedding_array.append(target_pred.numpy())
...
def kmeans(model, dataloader, label_sents):
    ...
    with paddle.no_grad():
        for source, label in tqdm(dataloader):
            ...
            embedding_array.append(source_pred.numpy())
            label_array.append(np.array(label))
...
def train(model, train_dl, test_dl, optimizer, label_sents):
    ...
    for batch_idx, source in enumerate(tqdm(train_dl), start=1):
        ...
        input_ids = source.get('input_ids').reshape([real_batch_num * 3, -1])
        attention_mask = source.get('attention_mask').reshape([real_batch_num * 3, -1])
        token_type_ids = source.get('token_type_ids').reshape([real_batch_num * 3, -1])
        # 训练
...
def get_similar_sents(sim_array, test_sents, test_labels, topk):
    ...
    for i in range(way):
        ...
        temp_tensor = sim_array[:,i][pred_array == i]
...
# train
if DO_TRAIN and DATA_TRAIN != None:
    best = 0
...
if DO_PREDICT:
    ...
    for i in range(1):
        ...
        selected_sents, selected_labels = kmeans_selected_sents, kmeans_selected_labels
        pesudo_data = get_pesudo_data(selected_sents, label_sents, 50 + i*50)
        pesudo_data = train_data + pesudo_data
        pesudo_dataloader = DataLoader(TrainDataset(pesudo_data), batch_size=BATCH_SIZE, shuffle=True)
        best = 0
